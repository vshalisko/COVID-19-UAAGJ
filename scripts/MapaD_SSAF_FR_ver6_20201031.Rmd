...
# ANALISIS JALISCO
## Defunciones por COVID-19 por fecha de reporte en datos SSA Federal
...
### Defunciones por COVID-19 confirmados en Jalisco
...
##### Defunciones durante la última semana (desde `r fecha_formato - 7` hasta `r fecha_formato`): **`r def_nac[1] - def_nac[8]`** (**`r round(100 * (def_nac[1] - def_nac[8]) / def_nac[8], 2)`%**).
##### Defunciones durante la semana anterior (desde `r fecha_formato - 15` hasta `r fecha_formato - 8`): **`r def_nac[8] - def_nac[15]`** (**`r round(100 * (def_nac[8] - def_nac[15]) / def_nac[15], 2)`%**).
##### Defunciones durante las últimas dos semanas (desde `r fecha_formato - 15` hasta `r fecha_formato`): **`r def_nac[1] - def_nac[15]`** (**`r round(100 * (def_nac[1] - def_nac[15]) / def_nac[15], 2)`%**).

### Resumen de defunciones confirmados por municipios de Jalisco y en los estados colindantes

```{r datos-para-tabla, echo = FALSE}
datos_municipio <- datos_municipio[datos_municipio[,casos_fechas[1]] > 0,]

datos_municipio$aumento_2sem <- datos_municipio[,def_fechas[1]] - datos_municipio[,def_fechas[15]]
datos_municipio$aumento_2semp <- round(100 * datos_municipio$aumento_2sem / datos_municipio[,def_fechas[15]],1)
...
datos_municipio$def_hoy100k <- round(100000 * datos_municipio[,def_fechas[1]] / datos_municipio$Pob_Municipio, 1)

## por 100k habitantes
casos_municipio_a100k <- round(100000 * datos_municipio[,def_fechas] / datos_municipio$Pob_Municipio, 1)
...
datos_municipio$dupl_2sem <- round(14 * log(2) / ((log(datos_municipio[,def_fechas[1]]) - log(datos_municipio[,def_fechas[15]]))),1)

vertor_order <- order(datos_municipio[,def_fechas[1]], decreasing = TRUE)
datos_municipio <- datos_municipio[vertor_order,]
casos_municipio_a100k <- casos_municipio_a100k[vertor_order,]
```
...
#str(datos_municipio)
 ...
 m2 <- addCircleMarkers(m2, lng = datos_municipio$Lon_Decimal, lat = datos_municipio$Lat_Decimal, 
                        weight = 5 * log(datos_municipio[,def_fechas[1]] + 1), radius = 0, 
                  color= "black", stroke = TRUE, fillOpacity = 1, group = "defunciones acumulados")
 m2 <- addLabelOnlyMarkers(m2, lng = datos_municipio$Lon_Decimal, lat = datos_municipio$Lat_Decimal, 
    label = filterHighLevel(datos_municipio[,def_fechas[1]]), group = "defunciones acumulados",
 ...
 m2 <- addCircleMarkers(m2, lng = datos_municipio$Lon_Decimal, lat = datos_municipio$Lat_Decimal, 
                        ...
                        weight = 5 * log(datos_municipio$def_hoy100k + 1), radius = 0, 
                  color= "darkred", stroke = TRUE, fillOpacity = 1, group = "defunciones acumulados por 100k")
 m2 <- addLabelOnlyMarkers(m2, lng = datos_municipio$Lon_Decimal, lat = datos_municipio$Lat_Decimal, 
    #label = filterHighLevel(casos_municipio_a100k[,def_fechas[1]]), group = "defunciones acumulados por 100k",
    label = filterHighLevel(round(datos_municipio$def_hoy100k,0)), group = "defunciones acumulados por 100k",
    labelOptions = labelOptions(noHide = T, direction = 'center', textOnly = T,
                    style = list("color" = "white","font-size" = "11px"))) 
 
 m2 <- addCircleMarkers(m2, lng = datos_municipio$Lon_Decimal, lat = datos_municipio$Lat_Decimal, 
                        weight = 5 * log(datos_municipio[,def_fechas[15]] + 1), radius = 0, 
                  color= "black", stroke = TRUE, fillOpacity = 1, group = "defunciones hace dos semanas")
 m2 <- addLabelOnlyMarkers(m2, lng = datos_municipio$Lon_Decimal, lat = datos_municipio$Lat_Decimal, 
    label = filterHighLevel(datos_municipio[,def_fechas[15]]), group = "defunciones hace dos semanas",
    labelOptions = labelOptions(noHide = T, direction = 'center', textOnly = T,
                    style = list("color" = "white","font-size" = "11px")))
 
 m2 <- addCircleMarkers(m2, lng = datos_municipio$Lon_Decimal, lat = datos_municipio$Lat_Decimal, 
                        weight = 10 * log(datos_municipio[,def_fechas[1]] - datos_municipio[,def_fechas[15]] + 1), radius = 0, 
                  color= "darkblue", stroke = TRUE, fillOpacity = 1, group = "aumento en dos semanas")
 m2 <- addLabelOnlyMarkers(m2, lng = datos_municipio$Lon_Decimal, lat = datos_municipio$Lat_Decimal, 
    label = filterHighLevel(datos_municipio[,def_fechas[1]] - datos_municipio[,def_fechas[15]]), group = "aumento en dos semanas",
    labelOptions = labelOptions(noHide = T, direction = 'center', textOnly = T,
                    style = list("color" = "white","font-size" = "11px"))) 
 ...
 m2 <- addLegendCustom(m2, 
   ...
   m2 <- addLegendCustom(m2, 
                      ...
                      group = "leyenda",
                      position = "bottomleft",
                      colors = c("darkred", "darkred", "darkred"), 
                      labels = c("1-10","10-20", "20-100"), 
                      sizes = c(
                                5 * log(10),
                                5 * log(20), 
                                5 * log(100)
 ...
 m2 <- addLayersControl(m2,
                       baseGroups = c("defunciones acumulados",
                                      "defunciones acumulados por 100k",
...
## Mapa de defunciones confirmados por COVID-19 por municipio en Jalisco y en los estados colindantes para `r fecha`
