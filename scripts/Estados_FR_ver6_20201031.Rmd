...
##### Aumento durante la última semana (desde `r fecha_formato - 6` hasta `r fecha_formato`): **`r casos_nac[1] - casos_nac[8]`** (**`r round(100 * (casos_nac[1] - casos_nac[8]) / casos_nac[8], 2)`%**).
##### Aumento durante la semana anterior (desde `r fecha_formato - 13` hasta `r fecha_formato - 7`): **`r casos_nac[8] - casos_nac[15]`** (**`r round(100 * (casos_nac[8] - casos_nac[15]) / casos_nac[15], 2)`%**).
##### Aumento durante la semana desde `r fecha_formato - 19` hasta `r fecha_formato - 14`: **`r format(casos_nac[15] - casos_nac[21], digits = 6)`** (**`r round(100 * (casos_nac[15] - casos_nac[21]) / casos_nac[21], 2)`%**).
##### Aumento durante las últimas dos semanas (desde `r fecha_formato - 13` hasta `r fecha_formato`): **`r casos_nac[1] - casos_nac[15]`** (**`r round(100 * (casos_nac[1] - casos_nac[15]) / casos_nac[15], 2)`%**).
  ...
  tryCatch({
    ...
    tryCatch({     
      ...
      if (length(smooth_Rt$R) > 0) {
        ...
        Rt_estado[i] <- round(tail(smooth_Rt$R, n=1),2)
...
DT::datatable(cbind(
                ...
                datos_estado[,c("aumento_hoyt","aumento_2semt", "dupl_2sem", "Rt")]
              ), 
         colnames = c("Estados","Hace 2 semanas","Hace 1 semana","Ayer","Hoy",
                      "Por 100k","Aumento desde ayer","Aumento en 2 semanas","TD, días","Rt"),
          ...
          options = list(scrollX = FALSE, 
                         ...
                         lengthMenu = c(5, 10, 15, 20),
                          ...
                          columnDefs = list(
                              ...
                              list(width = '30px', targets = c(9,10)),
                              list(className = 'dt-center', targets = c(2,3,4,5,6,7,8,9,10))
                          )                         
                         ))
```
...
cpen_barplot <- plot(x = 1:nrow(datos_estado), y = datos_estado$Rt, 
        ...
        ylab = "Rt", xlab = "", xaxt="n",
        main = paste("Número reproductivo de COVID-19 estimado para", fecha_formato))
