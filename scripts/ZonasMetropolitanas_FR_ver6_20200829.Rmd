---
title: "Tratamiento datos COVID-19 Zonas metropolitanas 29/08/2020"
...
datos_estado <- readRDS(paste0("object_metropolitanos_",fecha_formato,".rds"))
...
##### Aumento de casos desde ayer: **`r round(casos_nac[1] - casos_nac[2], 0)`** (**`r round(100 * (casos_nac[1] - casos_nac[2]) / casos_nac[2], 2)`%**). 
##### Aumento durante la última semana (desde `r fecha_formato - 7` hasta `r fecha_formato`): **`r format(casos_nac[1] - casos_nac[8], digits = 6)`** (**`r round(100 * (casos_nac[1] - casos_nac[8]) / casos_nac[8], 2)`%**).
##### Aumento durante la semana anterior (desde `r fecha_formato - 15` hasta `r fecha_formato - 8`): **`r format(casos_nac[8] - casos_nac[15], digits = 6)`** (**`r round(100 * (casos_nac[8] - casos_nac[15]) / casos_nac[15], 2)`%**).
##### Aumento durante las últimas dos semanas (desde `r fecha_formato - 15` hasta `r fecha_formato`): **`r format(casos_nac[1] - casos_nac[15], digits = 6)`** (**`r round(100 * (casos_nac[1] - casos_nac[15]) / casos_nac[15], 2)`%**).
...
mi_output_table <- cbind(
                datos_estado[,c("CVE_SUN","Estado", casos_fechas[15], casos_fechas[8], casos_fechas[2], casos_fechas[1])],
                casos_estado_a100k[,casos_fechas[1]],
                datos_estado[,c("aumento_2semt", "dupl_2sem","Rt")])
names(mi_output_table) <- c("CVE_SUN","NOM_SUN",casos_fechas[15], casos_fechas[8], casos_fechas[2], casos_fechas[1],
                      "Por_100k_hoy","Aumento_en_2_semanas","TD_dias","Rt")
write.csv(mi_output_table, file = paste("../datos_secundarios/casos_por_zm_",fecha,".csv", sep = ""), 
