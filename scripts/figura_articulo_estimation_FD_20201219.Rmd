...
# tiempo promedio desde sintomas hasta defucnion
t_defuncion <- 21
...
# Estimacion de casos potenciales a partir de datos sobre defunciones (FD)
...
metadatos_m <- datos_municipio[,1:12]
dia_m <- datos_municipio[,c(def_fechas)] / tasa_m2
dia_antes_m <- cbind(datos_municipio[,c(def_fechas[-1])],data.frame(inicio = rep(0,dim(datos_municipio)[1]))) / tasa_m2
incremento_d_m <- dia_m - dia_antes_m
incremento_d_m_a100k <- round(100000 * incremento_d_m / datos_municipio$Pob_Municipio, 3)
```
...
```{r}
mi_incremento_d_m <- as.matrix(incremento_d_m[,c(def_fechas_ok)])
colnames(mi_incremento_d_m) <- c(format(fecha_formato - seq(n-1, 0), "%d-%m-%Y"))

mi_incremento_d_m_a100k <- as.matrix(incremento_d_m_a100k[,c(def_fechas_ok)])
colnames(mi_incremento_d_m_a100k) <- c(format(fecha_formato - seq(n-1, 0), "%d-%m-%Y"))

municipios_order <- order(dia_m[,def_fechas[15]], decreasing = TRUE)
metadatos_ordenado_m <- metadatos_m[municipios_order,]
mi_incremento_d_m_ordenado <- mi_incremento_d_m[municipios_order,]
mi_incremento_d_m_a100k_ordenado <- mi_incremento_d_m_a100k[municipios_order,]
```
...
incremento_d_m_Jalisco <- apply(incremento_d_m, 2, sum, na.rm = TRUE)
incremento_d_m_a100k_Jalisco <- round(100000 * incremento_d_m_Jalisco / metadatos_m_J[1,"Pob_Municipio"], 3)
incremento_d_m_a100k_Jalisco <- as.data.frame(t(incremento_d_m_a100k_Jalisco))
#incremento_d_m_a100k_Jalisco <- incremento_d_m_a100k_Jalisco[,ncol(incremento_d_m_a100k_Jalisco):1]
colnames(incremento_d_m_a100k_Jalisco) <- c(format(fecha_formato - seq(n-1, 0), "%d-%m-%Y"))
incremento_d_m_a100k_Jalisco <- as.matrix(incremento_d_m_a100k_Jalisco[,ncol(incremento_d_m_a100k_Jalisco):1])
str(incremento_d_m_a100k_Jalisco)
#incremento_d_m_a100k_Jalisco <- cbind(metadatos_m_J, incremento_d_m_a100k_Jalisco)
#incremento_d_m_a100k_Jalisco
...
mi_grafica <- function (nivel, datos_1, datos_2, metadatos_1, modo = 1) {
  ...
  if (modo == 1) {
    titulo <- paste0("Casos de COVID-19 estimados a partir de defunciones en ",metadatos_1[nivel,6])
  } else {
    titulo <- ""
  }
  ...
  mi_barplot <- barplot(datos_1[nivel,1:length_visulal],
        ...
        horiz = FALSE, las = 1,
        col = "white",
        ylab = "casos diarias estimados por 100 mil habitantes", 
        names.arg = ""
        )
  ...
  lines(x = mi_x - t_defuncion, y = datos_1_average, col = "red", lwd = 2, lty = 1)
  lines(x = mi_x, y = datos_2_average, col = "blue", lwd = 2, lty = 1)
  
}

 #mi_grafica(nivel = 1, modo = 1,
 #           datos_1 = incremento_d_m_a100k_Jalisco, 
#            datos_2 = incremento_m_a100k_Jalisco,
#            metadatos_1 = metadatos_m_J)

mi_grafica(nivel = 1, 
           datos_1 = mi_incremento_d_m_a100k_ordenado, 
