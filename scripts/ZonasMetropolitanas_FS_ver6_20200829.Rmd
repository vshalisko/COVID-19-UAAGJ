...
# ANALISIS ZONAS METROPOLITANAS
## Casos positivos de COVID-19 por fecha de inicio de síntomas

```{r datos, echo = FALSE}
## lectura de datos
## hacer tabla de inicio con nombres de estados
datos_estado <- readRDS(paste0("object_metropolitanos_FS_",fecha_formato,".rds"))
datos_estado$Estado <- datos_estado$NOM_SUN
datos_estado$Poblacion <- datos_estado$POB_2010
```
...
### Casos de COVID-19 confirmados en zonas metropolitanas

Nota: Los datos por fecha de aparición de síntomas cuentan con un sesgo durante la semana más reciente. El sesgo está relacionado con el tiempo que pasa entre los síntomas de enfermedad, toma de muestra y recepción del resultado. Es por esto, los valores de aumento durante los días inmediatamente anteriores a la fecha del reporte son sistemáticamente más bajos de lo que realmente son.

##### Total de casos registrados para el día `r fecha_formato`: **`r format(casos_nac[1],digits = 7)`**. 
##### Casos con primeros síntomas presentados desde ayer: **`r round(casos_nac[1] - casos_nac[2], 0)`** (**`r round(100 * (casos_nac[1] - casos_nac[2]) / casos_nac[2], 2)`%**). 
##### Casos con primeros síntomas presentados durante la última semana (desde `r fecha_formato - 7` hasta `r fecha_formato`): **`r format(casos_nac[1] - casos_nac[8], digits = 6)`** (**`r round(100 * (casos_nac[1] - casos_nac[8]) / casos_nac[8], 2)`%**).
##### Casos con primeros síntomas presentados durante la semana anterior (desde `r fecha_formato - 15` hasta `r fecha_formato - 8`): **`r format(casos_nac[8] - casos_nac[15], digits = 6)`** (**`r round(100 * (casos_nac[8] - casos_nac[15]) / casos_nac[15], 2)`%**).
##### Casos con primeros síntomas presentados durante las últimas dos semanas (desde `r fecha_formato - 15` hasta `r fecha_formato`): **`r format(casos_nac[1] - casos_nac[15], digits = 6)`** (**`r round(100 * (casos_nac[1] - casos_nac[15]) / casos_nac[15], 2)`%**).

### Resumen de casos confirmados por zona metropolitana
...
tiempo de duplicación (TD) dos semanas antes de la fecha del reporte  
."

DT::datatable(cbind(
                datos_estado[,c("Estado", casos_fechas[15], casos_fechas[8], casos_fechas[2], casos_fechas[1])],
                casos_estado_a100k[,casos_fechas[1]],
                datos_estado[,c("aumento_2semt", "dupl_2sem")]
              ), 
         colnames = c("Zonas metropolitanas","Hace 2 semanas","Hace 1 semana","Ayer","Hoy",
                      "Por 100k","Aumento en 2 semanas","TD, días"),
         ...
         autoHideNavigation = TRUE,
          ...
          options = list(scrollX = FALSE, 
                         ...
                         lengthMenu = c(5, 10, 15, 20),
                          autoWidth = TRUE,
                          columnDefs = list(
                              list(width = '40px', targets = c(2,3,4,5,6,8)),
...
### Histogramas de total y aumento de casos positivos en 30 zonas metropolitanas con mayor incidencia {.tabset}
...
```{r echo = FALSE}
datos_estado_s <- datos_estado[order(datos_estado[,casos_fechas[1]], decreasing = TRUE),]
datos_estado_s <- datos_estado_s[1:30,]
datos_estado_s <- datos_estado_s[order(datos_estado_s$CVE_SUN),]
casos_estado_a100k_s <- round(100000 * datos_estado_s[,casos_fechas] / datos_estado_s$Poblacion, 1)
```
...
cpen_ymax <- 1.1 * max(datos_estado_s[,casos_fechas[1]])

cpen_barplot <- barplot(height = datos_estado_s[,casos_fechas[1]], 
        names.arg = datos_estado_s$Estado, col = "red",
...
barplot(height = datos_estado_s[,casos_fechas[15]], col = "gray", 
                        axes = FALSE, add = TRUE)

text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[1]] + 0.05 * cpen_ymax, 
     labels = datos_estado_s[,casos_fechas[1]], cex = 0.7)
text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[1]] + 0.02 * cpen_ymax, 
     labels = paste("+", datos_estado_s[,casos_fechas[1]] - datos_estado_s[,casos_fechas[15]], sep = ""), 
       col = "red", cex = 0.6)
...
barplot(height = datos_estado_s[,casos_fechas[8]], col = "gray", 
                        axes = FALSE, add = TRUE)

text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[1]] + 0.05 * cpen_ymax, 
     labels = datos_estado_s[,casos_fechas[1]], cex = 0.7)
text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[1]] + 0.02 * cpen_ymax, 
     labels = paste("+", datos_estado_s[,casos_fechas[1]] - datos_estado_s[,casos_fechas[8]], sep = ""), 
       col = "red", cex = 0.6)
...
cpen_ymax <- 1.1 * max(datos_estado_s[,casos_fechas[8]])

cpen_barplot <- barplot(height = datos_estado_s[,casos_fechas[8]], 
        names.arg = datos_estado_s$Estado, col = "red",
        las = 2, ylim = c(0, cpen_ymax),
        ylab = "casos de COVID-19",        
...
text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[8]] + 0.05 * cpen_ymax, 
     labels = datos_estado_s[,casos_fechas[8]], cex = 0.7)
text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[8]] + 0.02 * cpen_ymax, 
     labels = paste("+", datos_estado_s[,casos_fechas[8]] - datos_estado_s[,casos_fechas[15]], sep = ""), 
       col = "red", cex = 0.6)
...
barplot(height = datos_estado_s[,casos_fechas[2]], col = "gray", 
                        axes = FALSE, add = TRUE)

text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[1]] + 0.05 * cpen_ymax, 
     labels = datos_estado_s[,casos_fechas[1]], cex = 0.7)
text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[1]] + 0.02 * cpen_ymax, 
     labels = paste("+", datos_estado_s[,casos_fechas[1]] - datos_estado_s[,casos_fechas[2]], sep = ""), 
       col = "red", cex = 0.6)
...
### Histogramas de total y aumento de casos positivos por 100 mil habitantes en 30 zonas metropolitanas con mayor incidencia {.tabset}
...
cpen_ymax <- 1.1 * max(casos_estado_a100k_s[,casos_fechas[1]])

cpen_barplot <- barplot(height = casos_estado_a100k_s[,casos_fechas[1]], 
        names.arg = datos_estado_s$Estado, col = "red",
        las = 2, ylim = c(0, cpen_ymax),
...
barplot(height = casos_estado_a100k_s[,casos_fechas[15]], col = "gray", 
                        axes = FALSE, add = TRUE)

text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[1]] + 0.05 * cpen_ymax, 
     labels = casos_estado_a100k_s[,casos_fechas[1]], cex = 0.7)
text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[1]] + 0.02 * cpen_ymax, 
     labels = paste("+", round(casos_estado_a100k_s[,casos_fechas[1]] - casos_estado_a100k_s[,casos_fechas[15]],1), sep = ""), 
       col = "red", cex = 0.6)
...
barplot(height = casos_estado_a100k_s[,casos_fechas[8]], col = "gray", 
                        axes = FALSE, add = TRUE)

text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[1]] + 0.05 * cpen_ymax, 
     labels = casos_estado_a100k_s[,casos_fechas[1]], cex = 0.7)
text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[1]] + 0.02 * cpen_ymax, 
     labels = paste("+", round(casos_estado_a100k_s[,casos_fechas[1]] - casos_estado_a100k_s[,casos_fechas[8]],1), sep = ""), 
       col = "red", cex = 0.6)
...
cpen_ymax <- 1.1 * max(casos_estado_a100k_s[,casos_fechas[8]])

cpen_barplot <- barplot(height = casos_estado_a100k_s[,casos_fechas[8]], 
        names.arg = datos_estado_s$Estado, col = "red",
        las = 2, ylim = c(0, cpen_ymax),
        ylab = "casos COVID-19 por 100 mil habitantes",        
...
text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[8]] + 0.05 * cpen_ymax, 
     labels = casos_estado_a100k_s[,casos_fechas[8]], cex = 0.7)
text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[8]] + 0.02 * cpen_ymax, 
     labels = paste("+", round(casos_estado_a100k_s[,casos_fechas[8]] - casos_estado_a100k_s[,casos_fechas[15]],1), sep = ""), 
       col = "red", cex = 0.6)
...
barplot(height = casos_estado_a100k_s[,casos_fechas[2]], col = "gray", 
                        axes = FALSE, add = TRUE)

text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[1]] + 0.05 * cpen_ymax, 
     labels = casos_estado_a100k_s[,casos_fechas[1]], cex = 0.7)
text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[1]] + 0.02 * cpen_ymax, 
     labels = paste("+", round(casos_estado_a100k_s[,casos_fechas[1]] - casos_estado_a100k_s[,casos_fechas[2]],1), sep = ""), 
       col = "red", cex = 0.6)
...
### Dinámica de casos nuevos en últimas dos semanas en 30 zonas metropolitanas con mayor incidencia {.tabset}
...
La secuencia de particiones en las barras corresponde a la lista de zonas metropolitanas.
...
dia <- datos_estado_s[,c(casos_fechas)]
dia_antes <- cbind(datos_estado_s[,c(casos_fechas[-1])],data.frame(inicio = rep(0,nrow(datos_estado_s))))
incremento <- dia - dia_antes
...
legend("topright", legend = datos_estado_s[,"Estado"], fill=miscolores_vector, ncol = 7, bty = "n", inset=c(-0.05,-0.15), xpd=TRUE)
...
cpen_ymax <- 1.1 * max(casos_estado_a100k_s[,casos_fechas[1]] - casos_estado_a100k_s[,casos_fechas[15]])

cpen_barplot <- barplot(height = round(casos_estado_a100k_s[,casos_fechas[1]] - casos_estado_a100k_s[,casos_fechas[15]],1), 
        names.arg = datos_estado_s$Estado, col = "red",
        las = 2, ylim = c(0, cpen_ymax),
...
text(x = cpen_barplot, y = casos_estado_a100k_s[,casos_fechas[1]] - casos_estado_a100k_s[,casos_fechas[15]] + 0.02 * cpen_ymax, 
     labels = paste("+", round(casos_estado_a100k_s[,casos_fechas[1]] - casos_estado_a100k_s[,casos_fechas[15]],1), sep = ""), 
     cex = 0.7)
...
cpen_ymax <- 1.1 * max(datos_estado_s$aumento_2semp)

cpen_barplot <- barplot(height = datos_estado_s$aumento_2semp, 
        names.arg = datos_estado_s$Estado, col = "darkgray",
        las = 2, ylim = c(0, cpen_ymax),
...
text(x = cpen_barplot, y = datos_estado_s$aumento_2semp + 0.02 * cpen_ymax, 
     labels = paste(round(datos_estado_s$aumento_2semp,0), "%", sep = ""), 
     cex = 0.7)
#text(x = cpen_barplot, y = casos_estado_a100k[,casos_fechas[1]] + 0.02 * cpen_ymax, 
#     labels = paste("+", round(casos_estado_a100k[,casos_fechas[1]] - casos_estado_a100k[,casos_fechas[15]],1), sep = ""), 
#       col = "red", cex = 0.6)
...
cpen_ymax <- 1.1 * max(datos_estado_s$dupl_2sem)

cpen_barplot <- barplot(height = datos_estado_s$dupl_2sem, 
        names.arg = datos_estado_s$Estado, col = "beige",
        las = 2, ylim = c(0, cpen_ymax),
...
text(x = cpen_barplot, y = datos_estado_s$dupl_2sem + 0.02 * cpen_ymax, 
     labels = datos_estado_s$dupl_2sem, 
     cex = 0.7)
#text(x = cpen_barplot, y = casos_estado_a100k[,casos_fechas[1]] + 0.02 * cpen_ymax, 
#     labels = paste("+", round(casos_estado_a100k[,casos_fechas[1]] - casos_estado_a100k[,casos_fechas[15]],1), sep = ""), 
#       col = "red", cex = 0.6)
...
### Estimación del número de casos activos en 30 zonas metropolitanas con mayor incidencia {.tabset}
...
positividad_hace_semana <- (datos_estado_s[,casos_fechas[7]] - datos_estado_s[,casos_fechas[15]]) /
                            (datos_estado_s[,casos_fechas[7]] - datos_estado_s[,casos_fechas[15]] + 
                             datos_estado_s[,neg_fechas[7]] - datos_estado_s[,neg_fechas[15]])
...
cpen_ymax <- 1.1 * max(datos_estado_s[,casos_fechas[1]] - datos_estado_s[,casos_fechas[15]] + 
                         positividad_hace_semana * (datos_estado_s[,pend_fechas[8]] - datos_estado_s[,pend_fechas[15]]))

cpen_barplot <- barplot(height = datos_estado_s[,casos_fechas[1]] - datos_estado_s[,casos_fechas[15]] + 
                          positividad_hace_semana * (datos_estado_s[,pend_fechas[8]] - datos_estado_s[,pend_fechas[15]]), 
        names.arg = datos_estado_s$Estado, col = "yellow",
        las = 2, ylim = c(0, cpen_ymax),
        ylab = "casos activos de COVID-19",        
        main = paste("Estimación de casos de COVID-19 activos para", fecha_formato))

barplot(height = datos_estado_s[,casos_fechas[1]] - datos_estado_s[,casos_fechas[15]], col = "gray", 
                        axes = FALSE, add = TRUE)

text(x = cpen_barplot, y = datos_estado_s[,casos_fechas[1]] - datos_estado_s[,casos_fechas[15]] + 
       positividad_hace_semana * (datos_estado_s[,pend_fechas[8]] - datos_estado_s[,pend_fechas[15]]) + 0.03 * cpen_ymax, 
     labels = round(datos_estado_s[,casos_fechas[1]] - datos_estado_s[,casos_fechas[15]] + 
       positividad_hace_semana * (datos_estado_s[,pend_fechas[8]] - datos_estado_s[,pend_fechas[15]]),0), cex = 0.7)
...
cpen_barplot <- barplot(height = 100 * positividad_hace_semana, 
        names.arg = datos_estado_s$Estado, col = "lightblue",
        las = 2, ylim = c(0, 100),
