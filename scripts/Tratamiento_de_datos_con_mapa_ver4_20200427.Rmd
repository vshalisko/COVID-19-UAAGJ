---
title: "Tratamiento datos COVID-19 México 27/04/2020"
...
datos <- read.csv("200427COVID19MEXICO.csv")
datos_ayer <- read.csv("200426COVID19MEXICO.csv")
...
semana17 <- as.POSIXct('2020-04-19')
semana18 <- as.POSIXct('2020-04-26')
...
datos[datos$FECHA_SINTOMAS > semana17,"semana"] <- 18
datos[datos$FECHA_SINTOMAS > semana18,"semana"] <- 19
...
cpen_barplot <- barplot(height = cpen$casos, 
        ...
        las = 2, ylim = c(0, max(cpen$casos) + 300),
        main = paste("Total casos por estado para ", fecha))

barplot(height = cpen$casos_ayer, col = "gray", 
                        axes = FALSE, add = TRUE)

text(x = cpen_barplot, y = cpen$casos + 200, labels = cpen$casos, cex = 0.8)
text(x = cpen_barplot, y = cpen$casos + 80, labels = paste("+", cpen$aumento_casos), 
       col = "red", cex = 0.6)
...
dpen <- merge(x = dpen, y = poblacion_estados, by.x = "estados", by.y = "ID")
dpen$defunciones_por_100k <- 100000 * dpen$defunciones / dpen$Poblacion
...
dpen_barplot <- barplot(height = dpen$defunciones,
        ...
        las = 2, ylim = c(0, max(dpen$defunciones) + 25),
        main = paste("Total defunciones por estado para ", fecha))

barplot(height = dpen$defunciones_ayer, col = "gray",
                        axes = FALSE, add = TRUE)

text(x = dpen_barplot, y = dpen$defunciones + 15, labels = dpen$defunciones, cex = 0.8)
text(x = dpen_barplot, y = dpen$defunciones + 7, labels = paste("+", dpen$aumento_defunciones),
     col = "red", cex = 0.6)
```
...
npen <- merge(x = npen, y = dpen[,c("estados","defunciones","defunciones_ayer","aumento_defunciones","defunciones_por_100k")], by = "estados", all.x = TRUE)

npen$proporcion_defunciones <- npen$defunciones / npen$casos
...
colnames(hospitalizados_por_estado_um) <- c("estados","hospitalizados_concentrado")
...
colnames(uci_por_estado_um) <- c("estados","uci_concentrado")

npen <- merge(x = npen, y = hospitalizados_por_estado, by = "estados", all.x = TRUE)
npen <- merge(x = npen, y = uci_por_estado, by = "estados", all.x = TRUE)
npen <- merge(x = npen, y = hospitalizados_por_estado_um, by = "estados", all.x = TRUE)
npen <- merge(x = npen, y = uci_por_estado_um, by = "estados", all.x = TRUE)

npen$foraneos_hospitalizado <- npen$hospitalizados_concentrado - npen$hospitalizados
npen$foraneos_uci <- npen$uci_concentrado - npen$uci

npen$proporcion_hospitalizados <- npen$hospitalizados / npen$casos
npen$proporcion_uci <- npen$uci / npen$casos
...
npen$prevalencia_1 <- 100000 * npen$hospitalizados / (0.026 * npen$Poblacion)
...
npen$prevalencia_2 <- npen$defunciones_por_100k / 0.0053
...
tabla_npen <- npen[,c("estados","ENTIDAD_FEDERATIVA","Longitud","Latitud","Poblacion",
              "casos","casos_ayer","aumento_casos","casos_por_100k",
...
write.csv(tabla_npen, file = paste("casos_por_estado_analisis_",fecha,".csv", sep = ""), row.names = FALSE)
...
npen_barplot <- barplot(height = npen$pruebas,
        ...
        las = 2, ylim = c(0, max(npen$pruebas_ayer) + 2500),
        main = paste("Total pruebas por estado para ", fecha))

barplot(height = npen$pruebas_ayer, col = "gray",
                        axes = FALSE, add = TRUE)

text(x = npen_barplot, y = npen$pruebas + 800, labels = npen$pruebas, cex = 0.7)
