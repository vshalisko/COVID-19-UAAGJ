---
title: "Tratamiento datos COVID-19 Jalisco 21/08/2020"
...
#### Versión 6.0
#### Datos fuente
Datos abiertos del Gobierno de Jalisco "RADAR JALISCO" <https://coronavirus.jalisco.gob.mx/bases-de-datos/>
...
# ANALISIS JALISCO
## Casos positivos de COVID-19 por fecha de inicio de sintomas en datos RADAR JALISCO

```{r datos, echo = FALSE}
## lectura de datos
## hacer tabla de inicio con nombres de estados
datos_estado <- readRDS(paste0("object_estado_RJ_",fecha_formato,".rds"))
## hacer tabla de inicio con nombres de estados
datos_municipio <- readRDS(paste0("object_municipio_RJ_",fecha_formato,".rds"))
```
...
##### Casos con priemros sintomas presentados desde ayer: **`r round(casos_nac[1] - casos_nac[2], 0)`** (**`r round(100 * (casos_nac[1] - casos_nac[2]) / casos_nac[2], 2)`%**). 
##### Casos con primeros sintomas presentados durante la última semana (desde `r fecha_formato - 7` hasta `r fecha_formato`): **`r format(casos_nac[1] - casos_nac[8], digits = 6)`** (**`r round(100 * (casos_nac[1] - casos_nac[8]) / casos_nac[8], 2)`%**).
##### Casos con primeros sintomas presentados durante la semana anterior (desde `r fecha_formato - 15` hasta `r fecha_formato - 8`): **`r format(casos_nac[8] - casos_nac[15], digits = 6)`** (**`r round(100 * (casos_nac[8] - casos_nac[15]) / casos_nac[15], 2)`%**).
##### Casos con primeros sintomas presentados durante las últimas dos semanas (desde `r fecha_formato - 15` hasta `r fecha_formato`): **`r format(casos_nac[1] - casos_nac[15], digits = 6)`** (**`r round(100 * (casos_nac[1] - casos_nac[15]) / casos_nac[15], 2)`%**).
...
datos_municipio$dupl_2sem <- round(14 * log(2) / ((log(datos_municipio[,paste0("casos",fecha_cadena(fecha_formato - 15))]) - log(datos_municipio[,paste0("casos",fecha_cadena(fecha_formato - 29))]))),1)
...
for (i in 8:nrow(datos_municipio)) {
...
'
tabla1_pie <- "Tabla 1. Cambios en número acumulado de casos durante las últimas dos semanas,  
incidencia por 100 mil habitantes, número basico reproductivo (Rt) y tiempo de duplicación (TD)
dos semanas antes de la fecha del reporte. Nota: el calculo de Rt solo es posible en municipios con significativa acumulacion de casos."
...
DT::datatable(cbind(
         ...
         colnames = c("Municipios","Hace 2 semanas","Hace 1 semana","Ayer","Hoy",
                      "Por 100k","Aumento en 2 semanas","TD, dias","Rt"
                      ),
...
```{r echo = FALSE}
knitr::asis_output(htmltools::htmlPreserve('<div class="caption">Figura 1. Barras grices y números negros representan 
los el casos acumulado antes de la fecha del analisis de aumento, 
las barras y números rojos aumento durnte el periodo considerado.</div><br />'))
```
...
```{r echo = FALSE}
knitr::asis_output(htmltools::htmlPreserve('<div class="caption">Figura 2. Barras grices y números negros representan 
los el casos acumulado antes de la fecha del analisis de aumento, 
las barras y números rojos aumento durnte el periodo considerado.</div><br />'))
```
...
legend("topright", legend = datos_municipio[,"Nom_Mun"], fill=miscolores_vector, ncol = 5, bty = "n", inset=c(-0.1,-0.15), xpd=TRUE)
...
cpen_ymax <- 1.1 * max(casos_municipio_a100k[,casos_fechas[1]] - casos_municipio_a100k[,casos_fechas[15]])

cpen_barplot <- barplot(height = round(casos_municipio_a100k[,casos_fechas[1]] - casos_municipio_a100k[,casos_fechas[15]],1),
        names.arg = datos_municipio$Nom_Mun, col = "red",
        las = 2, ylim = c(0, cpen_ymax),
...
text(x = cpen_barplot, y = casos_municipio_a100k[,casos_fechas[1]] - casos_municipio_a100k[,casos_fechas[15]] + 0.02 * cpen_ymax,
     labels = paste("+", round(casos_municipio_a100k[,casos_fechas[1]] - casos_municipio_a100k[,casos_fechas[15]],1), sep = ""),
     cex = 0.7)
#text(x = cpen_barplot, y = casos_municipio_a100k[,casos_fechas[1]] + 0.02 * cpen_ymax,
#     labels = paste("+", round(casos_municipio_a100k[,casos_fechas[1]] - casos_municipio_a100k[,casos_fechas[15]],1), sep = ""),
...
cpen_barplot <- barplot(height = datos_municipio$dupl_2sem, 
        ...
        las = 2, ylim = c(0, cpen_ymax),
        ylab = "tiempo de duplicación de número de casos (dias)",
        main = paste("Tiempo de duplicación de casos de COVID-19 observado entre", fecha_formato - 30, "y", fecha_formato - 15))
